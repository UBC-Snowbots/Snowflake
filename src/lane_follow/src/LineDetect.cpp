...
LineDetect::getLanePoints(cv::Mat &filtered_image) {
    ...
    for (int window_slice_index = 0; window_slice_index < num_vertical_slices;
        ...
        for (int window_index = 0; window_index < base_windows.size(); window_index++) {

            // obtain the base window
            Window BaseWindow = base_windows.at(static_cast<unsigned long>(window_index));
            ...
            int y = (window_slice_index * filtered_image.rows / num_vertical_slices)
                    + window_slice.rows / 2;
...
std::vector<Window> LineDetect::getBaseWindows(cv::Mat &filtered_image) {
    ...
    std::pair<int, int> peaks = this->getBaseHistogramPeakPositions(base_histogram);
...
std::pair<int, int> LineDetect::getBaseHistogramPeakPositions(intvec base_histogram) {
    ...
    for (auto i = static_cast<int>(base_histogram.size() / 2); i < base_histogram.size(); i++) {
...
cv::Mat LineDetect::getWindowSlice(cv::Mat &filtered_image,
    ...
    cv::Mat window_slice = filtered_image(
            ...
            Range(vertical_slice_index * filtered_image.rows / num_vertical_slices,
                  (vertical_slice_index + 1) * filtered_image.rows / num_vertical_slices),
