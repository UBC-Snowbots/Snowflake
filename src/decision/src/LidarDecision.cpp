...
geometry_msgs::Twist LidarDecision::twist_message_from_obstacle(LidarObstacle obstacle,
    ...
    if (obstacle.getMinDistance() <= danger_distance && obstacle.getAvgAngle() <= danger_angle &&
        obstacle.getAvgAngle() >= -danger_angle) {
        float minDistance = obstacle.getMinDistance();
        twist.linear.x = sqrt(minDistance);
        // Calculate angular z based on how aligned obstacle is to the robot
        float angle_dep_score = abs(cos(obstacle.getAvgAngle()));
        // Calculate angular z based on how close obstacle is to the robot
        float distance_dep_score = 1 / minDistance;
        // Check if obstacle is more dangerous because of its angle or distance relative to the robot
        if (abs(angle_dep_score) >= abs(distance_dep_score))
            twist.angular.z = angular_vel_multiplier * angle_dep_score;
        else {
        // If distance score is too large robot will oversteer, so just set as the multiplier value
        // so multiplier value should be set to 1.5, which translates angular z to ~90 degree turn
            twist.angular.z = (distance_dep_score > 1) ? linear_vel_multiplier :
                              linear_vel_multiplier * distance_dep_score;
        }
