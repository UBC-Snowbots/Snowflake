...
Polynomial Polynomial::deriv(unsigned int degree) {
    ...
    for (unsigned int i = degree; i < _coefficients.size(); i++){
        new_coeffs.emplace_back(
                (boost::math::factorial<double>(i) / boost::math::factorial<double>(i-degree))
                *_coefficients[i]);
    }
...
double Polynomial::deriv(double x, unsigned int degree) {
    double y = 0;
    for (unsigned int i = degree; i < _coefficients.size(); i++){
        y += (boost::math::factorial<double>(i) / boost::math::factorial<double>(i-degree))
