 ...
 * it into a 2D domain.
 * i.e. Turning a video of the environment into
...
void IPM::drawPoints(const std::vector<cv::Point2f>& _points,
                     cv::Mat& _img) const {
...
void IPM::applyHomographyInv(const Mat& _inputImg,
                             Mat& _dstImg,
                             int _borderMode) {
    // Generate IPM image from src
    remap(_inputImg,
