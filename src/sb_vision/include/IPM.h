...
class IPM {
  public:
    ...
    void applyHomography(const cv::Mat& _origBGR,
                         cv::Mat& _ipmBGR,
                         int borderMode = cv::BORDER_CONSTANT);
    void applyHomographyInv(const cv::Mat& _ipmBGR,
                            cv::Mat& _origBGR,
                            int borderMode = cv::BORDER_CONSTANT);
    ...
    void getPoints(std::vector<cv::Point2f>& _origPts,
                   std::vector<cv::Point2f>& _ipmPts);

    // Draw
    void drawPoints(const std::vector<cv::Point2f>& _points,
                    cv::Mat& _img) const;
