...
RiskAnalysisNode::RiskAnalysisNode(int argc, char **argv, std::string node_name) {
    // Setup NodeHandles
    ...
    SB_getParam(private_nh, "area_of_interest_width", area_of_interest_width, default_width);

    float default_height                = 5;
    SB_getParam(private_nh, "area_of_interest_height", area_of_interest_height, default_height);

    int default_vertical_divisions    = 10;
    SB_getParam(private_nh, "num_vertical_cell_div", num_vertical_cell_div, default_vertical_divisions);

    int default_horizontal_divisions  = 30;
    SB_getParam(private_nh, "area_of_interest_width", num_horizontal_cell_div, default_horizontal_divisions);

    int default_min_points = 100;
    SB_getParam(private_nh, "region_min_points", region_min_points, default_min_points);
    ...
    for (int i = 0; i < gradient_size; i++) {
        gradient[i] = snowbots::RvizUtils::createMarkerColor(i*gradient_step, gradient_step*(gradient_size - i), 0, 1.0f);
    }
    ...
    std::string risk_areas_topic =
            "output_risk_areas"; // dummy topic name
    ...
    risk_marker_publisher   = private_nh.advertise<visualization_msgs::MarkerArray>(
            risk_area_markers_topic, queue_size);

    // Initialise risk_analysis with parameters
    risk_analysis = RiskAnalysis(area_of_interest_width, area_of_interest_height, num_vertical_cell_div, num_horizontal_cell_div, region_min_points);

}

void RiskAnalysisNode::pclCallBack(const sensor_msgs::PointCloud2ConstPtr point_cloud) {
...
visualization_msgs::Marker::_color_type RiskAnalysisNode::convertRiskToColor(float risk) {
