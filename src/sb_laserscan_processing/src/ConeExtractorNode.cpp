 ...
 * Implementation for a Cone Extractor Node that identifies cones from a laser
 * msg
 */

#include <ConeExtractorNode.h>

ConeExtractorNode::ConeExtractorNode(int argc,
                                     char** argv,
    ...
    SB_getParam(
    private_nh, cone_dist_tol_param, cone_dist_tol, default_cone_dist_tol);

    std::string cone_rad_exp_param = "cone_rad_exp";
    double default_cone_rad_exp    = 0.3;
    SB_getParam(
    private_nh, cone_rad_exp_param, cone_rad_exp, default_cone_rad_exp);

    std::string cone_rad_tol_param = "cone_rad_tol";
    double default_cone_rad_tol    = 0.1; // Change later
    SB_getParam(
    private_nh, cone_rad_tol_param, cone_rad_tol, default_cone_rad_tol);
    ...
    subscribe_topic, queue_size, &ConeExtractorNode::laserCallBack, this);
    ...
    rviz_publisher =
    private_nh.advertise<visualization_msgs::Marker>(marker_topic, queue_size);
}

void ConeExtractorNode::laserCallBack(
const sensor_msgs::LaserScan::ConstPtr& ptr) {
...
void ConeExtractorNode::publishMarkers(
std::vector<mapping_igvc::ConeObstacle> cones) {
