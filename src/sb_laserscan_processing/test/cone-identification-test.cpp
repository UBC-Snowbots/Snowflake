...
TEST(ConeIdentification, edgeToCone3Points) {
    ...
    mapping_igvc::Point2D p1;
    p1.x = -2;
    p1.y = 2;
    mapping_igvc::Point2D p2;
    p2.x = 0;
    p2.y = 0;
    mapping_igvc::Point2D p3;
    p3.x = 2;
    p3.y = 2;
    ...
    ConeIdentification::edgeToCone(edge_points);
    EXPECT_FLOAT_EQ(cone.center.x, 0);
    EXPECT_FLOAT_EQ(cone.center.y, 2);
    EXPECT_FLOAT_EQ(cone.radius, 2);
}

// Test 6 points->cone in an edge (forming a semi circle) - points are
...
TEST(ConeIdentification, edgeToCone6Points) {
    ...
    p3.x = -1;
    p3.y = 0.268;
    mapping_igvc::Point2D p4;
    p4.x = 1;
    p4.y = 0.268;
    mapping_igvc::Point2D p5;
    p5.x = 1.73;
    p5.y = 1;
    mapping_igvc::Point2D p6;
    p6.x = 2;
    p6.y = 2;
    edge_points.push_back(p1);
    edge_points.push_back(p2);
...
// Test 4 points->cone in an edge (forming a quarter circle/arc) - points are
// evenly distributed
...
// Test identifyCones with no valid cones in image (there is one valid cone but
// out of range)
TEST(ConeIdentification, coneOutRange) {
    ...
    ConeIdentification::identifyCones(laser_msg,
                                      dist_tol,
                                      radius_exp,
                                      radius_tol,
                                      min_points_in_cone,
...
// Test identifyCones with no valid cones in image (there is one object with
// radius too large, of 1.5m instead of 1m)
...
// Test identifyCones with 2 cones NOT in a cluster (they are seperate from each
// other)
...
// Test identifyCones with 4 cones NOT in a cluster (they are seperate from each
// other)
TEST(ConeIdentification, fourValidCones) {
    ...
    builder.addCone(0, -3, 0.5); // ISSUE WITH THIS (Appears to be bug with
                                 // detecting cones along the negative y line)
    laser_msg = builder.getLaserscan();
...
// Test identifyCones with 2 cones in a cluster (their laserscan points
// "connect", horizontal relative to the robot)
...
// Test identifyCones with 2 cones in a diagonal orientation (though not
