...
LidarDecision::LidarDecision(int argc, char** argv, std::string node_name) {
    ...
    scan_subscriber                  = public_nh.subscribe(
    ...
    // Get Param(s)
    SB_getParam(public_nh,
                "max_obstacle_angle_diff",
                max_obstacle_angle_diff,
                (float) M_PI / 36);
    SB_getParam(public_nh,
                "max_obstacle_danger_distance",
                max_obstacle_danger_distance,
                (float) 10);
    SB_getParam(public_nh,
                "obstacle_danger_angle",
                obstacle_danger_angle,
                (float) M_PI / 4);
...
void LidarDecision::scanCallBack(
...
geometry_msgs::Twist LidarDecision::generate_twist_message(
const sensor_msgs::LaserScan::ConstPtr& raw_scan) {
    // Find all the obstacles
    std::vector<LidarObstacle> obstacles = findObstacles(
    *raw_scan, max_obstacle_angle_diff, max_obstacle_distance_diff);

    // Check that we have at least one obstacle
    if (obstacles.size() >= 1) {
        // Choose the most dangerous obstacle
        LidarObstacle most_dangerous_obstacle =
        mostDangerousObstacle(obstacles);

        // Create and return a twist message based on the most dangerous
        // obstacle
        return twist_message_from_obstacle(most_dangerous_obstacle,
                                           max_obstacle_danger_distance,
                                           obstacle_danger_angle,
                                           twist_velocity,
                                           twist_turn_rate);
...
std::vector<LidarObstacle>
LidarDecision::findObstacles(const sensor_msgs::LaserScan& scan,
                             float max_obstacle_angle_diff,
                             float max_obstacle_distance_diff) {
    // Get the raw scan data
    std::vector<float> scan_data = scan.ranges;

    // Find all ranges (lidar hits) that could be obstacles (initially each
    // laser hit is an obstacle)
    ...
    for (int i = 0; i < scan_data.size(); i++) {
        ...
        if (scan_data[i] < scan.range_max && scan_data[i] > scan.range_min) {
            ...
            obstacles.emplace_back(LidarObstacle(
            scan.angle_increment * i + scan.angle_min, scan_data[i]));
        }
    }

    // Merge together obstacles that could be the same obstacle
    mergeSimilarObstacles(
    obstacles, max_obstacle_angle_diff, max_obstacle_distance_diff);

    return obstacles;
}

LidarObstacle LidarDecision::mostDangerousObstacle(
const std::vector<LidarObstacle> obstacles) {
    // Return obstacle with the greatest danger score
    return *std::max_element(obstacles.begin(),
                             obstacles.end(),
...
void LidarDecision::mergeSimilarObstacles(std::vector<LidarObstacle>& obstacles,
    ...
    while (i < (long) obstacles.size() - 1) {
        // Check if angle difference between two consecutive scans is less than
        // max_angle_diff and max_distance_diff
        if (fabs(obstacles[i + 1].getMinAngle() - obstacles[i].getMaxAngle()) <
            max_angle_diff &&
            fabs(obstacles[i + 1].getFirstDistance() -
                 obstacles[i].getLastDistance()) < max_distance_diff) {
...
geometry_msgs::Twist
LidarDecision::twist_message_from_obstacle(LidarObstacle obstacle,
    ...
    if (obstacle.getMinDistance() <= danger_distance &&
        abs(obstacle.getAvgAngle()) <= danger_angle) {
        ...
        if (angle_score >= distance_score)
            ...
            // If distance score is too large robot will oversteer and come back
            // around, so just set as
            // multiplier value instead (should be set to 1.5). This translates
            // angular z to ~90° turn
            twist.angular.z = (distance_score > 1)
                              ? linear_vel_multiplier
                              : linear_vel_multiplier * distance_score;
        }
        // Check if we should be turning left or right away from obstacle
        if (obstacle.getAvgAngle() > 0) twist.angular.z *= -1;
    }
    return twist;
}
