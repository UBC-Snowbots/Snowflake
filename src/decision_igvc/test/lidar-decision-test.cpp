...
class LidarDecisionTest : public testing::Test {
  protected:
    virtual void SetUp() {
        ...
        // Add the obstacles
        std::fill(
        scan1.ranges.begin(), scan1.ranges.begin() + 10, 36.123); // obstacle 1
        std::fill(scan1.ranges.begin() + 10,
                  scan1.ranges.begin() + 20,
                  38.123); // obstacle 2
        std::fill(scan1.ranges.begin() + 100,
                  scan1.ranges.begin() + 105,
                  24.123); // obstacle 3
        std::fill(scan1.ranges.begin() + 105,
                  scan1.ranges.begin() + 110,
                  26.123); // obstacle 4
        std::fill(scan1.ranges.begin() + 125,
                  scan1.ranges.begin() + 130,
                  25.123); // obstacle 5
        std::fill(scan1.ranges.begin() + 200,
                  scan1.ranges.begin() + 240,
                  32); // obstacle 6
        // Make some ranges outside the min and max of the scan
        std::fill(
        scan1.ranges.begin() + 110, scan1.ranges.begin() + 125, 45.123);
        ...
        // Add the obstacles
        std::fill(
        scan2.ranges.begin(), scan2.ranges.begin() + 10, 37.123); // obstacle 1
        std::fill(scan2.ranges.begin() + 15,
                  scan2.ranges.begin() + 20,
                  37.123); // obstacle 2
        std::fill(scan2.ranges.begin() + 100,
                  scan2.ranges.begin() + 105,
                  24.123); // obstacle 3
        std::fill(scan2.ranges.begin() + 105,
                  scan2.ranges.begin() + 110,
                  26.123); // obstacle 4
        std::fill(scan2.ranges.begin() + 125,
                  scan2.ranges.begin() + 130,
                  25.123); // obstacle 5
        std::fill(scan2.ranges.begin() + 200,
                  scan2.ranges.begin() + 240,
                  32); // obstacle 6
        // Make some ranges outside the min and max of the scan
        std::fill(
        scan2.ranges.begin() + 110, scan2.ranges.begin() + 125, 45.123);
...
TEST_F(LidarDecisionTest, findObstaclesDistanceTest) {
    ...
    LidarDecision::findObstacles(scan1, 0.02, 1.0);
    // The distance separates obstacle1 and obstacle2 making the size to be 6
    // instead of 5
...
TEST_F(LidarDecisionTest, findObstaclesAngleTest) {
    std::vector<LidarObstacle> found_obstacles =
    LidarDecision::findObstacles(scan2, 0.02, 1.0);
    // The angle separates obstacle1 and obstacle2 making the size to be 6
    // instead of 5
    EXPECT_EQ(6, found_obstacles.size());
    // Check that all obstacles are at acceptable ranges
    for (LidarObstacle obstacle : found_obstacles) {
...
TEST_F(LidarDecisionTest, angular_twist_message_from_obstacleTest) {
    ...
    EXPECT_NEAR(1.0606602,
                ...
                LidarObstacle(-M_PI / 4, 3.0), 4.0, M_PI / 2, 1.5, 1.5)
                .angular.z,
                0.000001);
    EXPECT_NEAR(1.5,
                LidarDecision::twist_message_from_obstacle(
                LidarObstacle(0.0, 4.7), 5.0, M_PI / 2, 1.5, 1.5)
                .angular.z,
                0.000001);
    EXPECT_NEAR(-1.5,
                LidarDecision::twist_message_from_obstacle(
                LidarObstacle(M_PI / 2, 0.9), 2.0, M_PI / 2, 1.5, 1.5)
                .angular.z,
                0.000001);
}
...
TEST_F(LidarDecisionTest, linear_twist_message_from_obstacleTest) {
    ...
    EXPECT_NEAR(0.0,
                LidarDecision::twist_message_from_obstacle(
                LidarObstacle(3 * M_PI / 4, 2.1), 3.0, M_PI / 2, 1.5, 1.5)
                .linear.x,
                0.000001);
    EXPECT_NEAR(1.7320508,
                LidarDecision::twist_message_from_obstacle(
                LidarObstacle(-M_PI / 3, 3.0), 4.0, M_PI / 2, 1.5, 1.5)
                .linear.x,
                0.000001);
    EXPECT_NEAR(0.6324555,
                LidarDecision::twist_message_from_obstacle(
                LidarObstacle(0.0, 0.4), 5.0, M_PI / 2, 1.5, 1.5)
                .linear.x,
                0.000001);
}
