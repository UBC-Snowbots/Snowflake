...
class LidarDecision {
  public:
    ...
    /**
     * Creates a twist command from a given lidar scan that will best avoid
     * obstacles in the scan
     *
     * @return A twist command from a given lidar scan that will best avoid
     * obstacles in the scan
     */
    geometry_msgs::Twist
    generate_twist_message(const sensor_msgs::LaserScan::ConstPtr& raw_scan);

    /**
     * Finds all obstacles in a given scan
     *
     * @param sensor_msgs\LaserScan a laser scan in which to find obstacles
     * @param max_obstacle_angle_diff the maximum possible angular difference
     * between two lidar hits
     *          for them to be considered the same obstacle
     *
     * @return a vector of all obstacles
     */
    static std::vector<LidarObstacle>
    findObstacles(const sensor_msgs::LaserScan& scan,
     ...
     */
    static LidarObstacle
    mostDangerousObstacle(const std::vector<LidarObstacle> obstacles);
     ...
     * @param max_angle_diff the max difference that two obstacles may differ by
     * and still be considered
     *                      part of the same obstacle
     * @param max_distance_diff the max difference that two obstacles may differ
     * by and still be considered
     *                      part of the same obstacle
     */
     ...
     *
     * @param max_obstacle_danger_distance distance which the obstacle must be
     * within to be considered
     * @param obstacle the obstacle to be considered
     *
     * @return a twist message
     */
    static geometry_msgs::Twist
    twist_message_from_obstacle(LidarObstacle obstacle,
  ...
  private:
    ...
    // The maximum angle which two scans can be different by to be considered
    // the same obstacle
    float max_obstacle_angle_diff;
    // The maximum di which two scans can be different by to be considered the
    // same obstacle
    float max_obstacle_distance_diff;
    // The distance at which an obstacle can be and be considered a danger
    float max_obstacle_danger_distance;
    // The angle, measured from 0 being directly in front of the robot, at which
    // an obstacle
    // is considered a danger to the robot
    float obstacle_danger_angle;
    // The multiplier for angular velocity that that robot uses to create twist
    // messages
    float twist_turn_rate;
    // The multiplier for linear velocity that that robot uses to create twist
    // messages
