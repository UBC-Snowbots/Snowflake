...
TEST(ReactiveSystem, straightTraj) {
    ...
    std::vector<sb_geom_msgs::Point2D> traj = ReactiveSystemPath::getArcTrajectory(linear_vel, angular_vel, time_inc, num_incs);
...
TEST(ReactiveSystem, riskAreaBuilder) {
    ...
    builder.addRiskZone(risk_center, risk_radius, risk_score); //rad = 1, score = 5
    ...
    for (mapping_msgs_urc::RiskArea area : risks.areas){
        if (area.score.data == risk_score) { //confirm that risk scores were added in correct positions
            EXPECT_NEAR(pow(area.area.points[0].x - risk_center.x, 2) + pow(area.area.points[0].y - risk_center.y, 2), 0, pow(risk_radius, 2) + 0.15);
        }
...
//Test the given twist, for a scenario where an object is directly between our position and the goal
TEST(ReactiveSystem, getArcPath1) {
    ...
    nav_msgs::Path path = ReactiveSystemPath::getPath(risk_areas, goal, time_inc, num_incs, linear_vel, max_angular_vel, num_angular_vel, risk_dist_tol_sq);

    float path_ang = getAvgAngle(path);
    EXPECT_GE(fabs(path_ang), fabs(atan2(risk_center.y + risk_radius, risk_center.x)));

}

//Test the given twist, for a scenario where 3 objects are directly between our position and the goal (covering a greater "width)
TEST(ReactiveSystem, getArcPath2) {
    ...
    float path_ang = getAvgAngle(path);
    EXPECT_GE(fabs(path_ang), fabs(atan2(risk_center2.y + risk_radius, risk_center2.x)));
...
//Test the given twist, for a scenario where there are two objects but a gap exists in the center
...
//Test the given twist, for a scenario where there are two objects but a space exists to the "right"
...
//Test the given twist, for a scenario where there are three objects but a gap exists between two objects to the "right"
...
//Test the given twist, for a scenario where no objects are in the way at all, but goal is not directly straight
...
//Test the given twist, for a scenario where there are three objects but a gap exists between two objects to the "right", BUT goal is to the left behind the objects
