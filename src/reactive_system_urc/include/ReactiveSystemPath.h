 ...
 * This node is responsible for moving the robot towards a given GPS goal while
...
class ReactiveSystemPath {
  public:
    /**
     * Entry point for pathfinding algorithm, produces a path message by
    ...
    /**
     * Calculates the arc trajectory given some twist command (linear and
     * angular velocity), form of trajectory is point locations of the
     * trajectory
     * given at discrete time steps , and coordinates are in the frame of robot
     * position
     * @param linear_vel: linear x velocity of trajectory
     * @param angular_vel: angular z velocity of trajectory
     * @param time_inc: time increment step in seconds, dictates the distance
     * between trajectory points (should not need to be too small)
     * @param num_incs: number of point positions in trajectory (should not be
     * too high but >= 1)
     * @return list of point positions
     */
    static std::vector<sb_geom_msgs::Point2D> getArcTrajectory(
     ...
     */
    static sb_geom_msgs::Point2D getArcPoint(sb_geom_msgs::Point2D center,
                                             float angle);

    /**
     * Calculate the risk score for a given trajectory, where higher scores are
     * riskier
     * @param trajectory: a proposed trajectory for the robot
     * @param goal_pos: the current goal heading for the robot (in robot frame)
     * @param risk_areas
     * @param dist_tol_sq: the maximum distance for any given risk area to be
     * within some trajectory point (should change based on robot dimensions),
     * SQUARED
     * @return a risk score for the given trajectory
     */
    static float
    getTrajectoryScore(std::vector<sb_geom_msgs::Point2D> trajectory,
                       sb_geom_msgs::Point2D goal_pos,
