...
std::vector<std::vector<geometry_msgs::Polygon>> DBScanPoly::findClusters(std::vector<geometry_msgs::Polygon> polygons, double threshold){
    ...
    for (int i=0; i<polygons.size(); i++) {
        if (!is_clustered[i]){
            ...
            expand(polygons, cluster, seed_poly, i, is_clustered, threshold);
...
void DBScanPoly::expand(std::vector<geometry_msgs::Polygon> &polygons, std::vector<geometry_msgs::Polygon> &cur_cluster, geometry_msgs::Polygon target_poly, int seed_index, std::vector<bool> &is_clustered, double threshold) {

    //Find neighbors of target_polygon
    std::vector<geometry_msgs::Polygon> neighbors;
    for (int i = seed_index + 1; i < polygons.size(); i++) {
        if (!is_clustered[i] && arePolygonsClose(target_poly, polygons[i], threshold)) { //Neighbor found
            is_clustered[i] = true;
    ...
    for (geometry_msgs::Polygon new_target_poly : neighbors) {
        expand(polygons, cur_cluster, new_target_poly, seed_index, is_clustered, threshold);
    }
}
...
bool DBScanPoly::arePolygonsClose(geometry_msgs::Polygon poly1, geometry_msgs::Polygon poly2, double threshold){
    ...
    for (geometry_msgs::Point32 point1 : poly1.points){
        for (geometry_msgs::Point32 point2 : poly2.points){
            ...
            if (pow((point1.x - point2.x), 2) + pow((point1.y - point2.y), 2) <= pow(threshold, 2))
                return true;
