...
float dist_tol_sq) {
    ...
    for (int i = 0; i < trajectory.size(); i++) {
        ...
        float traj_risk = -1; // risk score for a given trajectory point, set to
        ...
        for (mapping_msgs_urc::RiskArea area :
            ...
            for (sb_geom_msgs::Point2D risk_point : area.area.points) {
                if (isWithinDistance(risk_point, traj_point, dist_tol_sq)) {
                    if (area.score.data > traj_risk) // Keep track of the
    ...
    float traj_ang =
    atan2(trajectory[trajectory.size() - 1].y,
          trajectory[trajectory.size() - 1].x); // approximate trajectory angle
                                                // from the line b/w last point
                                                // in the trajectory and origin
    float ang_diff   = abs(goal_ang - traj_ang);
    float goal_adder = ang_diff * 100;
