...
nav_msgs::Path ReactiveSystemPath::getPath(mapping_msgs_urc::RiskAreaArray risk_areas, sb_geom_msgs::Point2D goal_pos, float traj_time_inc, int traj_num_incs, float linear_vel, float max_angular_vel, int num_angular_vel, float risk_dist_tol_sq) {
...
std::vector<sb_geom_msgs::Point2D> ReactiveSystemPath::getArcTrajectory(float linear_vel, float angular_vel, float time_inc, int num_incs){
    ...
    if (abs(angular_vel) < 0.01){ //essentially a straight trajectory
        ...
        for (int i = 1; i <= num_incs; i++){
            ...
            sb_geom_msgs::Point2D arc_point = ReactiveSystemPath::getArcPoint(center, angle);
            arc_trajectory.push_back(arc_point);
        }
...
sb_geom_msgs::Point2D ReactiveSystemPath::getArcPoint(sb_geom_msgs::Point2D center, float angle){
...
float ReactiveSystemPath::getTrajectoryScore(std::vector<sb_geom_msgs::Point2D> trajectory, sb_geom_msgs::Point2D goal_pos, mapping_msgs_urc::RiskAreaArray risk_areas, float dist_tol_sq){

    float risk_score = 0;

    //Give each point in the trajectory a riskiness based on proximity to risk areas and immediate proximity to current position
    ...
    for (int i=0; i<trajectory.size(); i++){
        ...
        traj_risk *= (trajectory.size() - i); //TODO: Don't know if we should keep this
        risk_score += traj_risk;
    }

    //Apply an additive value to the total risk score for a given trajectory, based on how close it heads towards the goal (closer = lower multiplier)
    ...
    float goal_adder = ang_diff * 100; //TODO: may need to tweak this scaling factor
...
bool ReactiveSystemPath::isWithinDistance(sb_geom_msgs::Point2D p1, sb_geom_msgs::Point2D p2, float dist_tol_sq){
