...
OccupancyGridConversionService OccupancyGridConversionService::buildService(nav_msgs::MapMetaData info) {
    ...
    tf::Vector3 origin_position = PathFinderUtils::pointToVector(info.origin.position);
    ...
    service._transformation_service = FrameTransformationService::buildService(origin_quaternion, origin_position);
    service._grid_info = info;

    return service;
}

AStar::GridPoint OccupancyGridConversionService::convertToGridPoint(geometry_msgs::Point point) {
    geometry_msgs::Point point_in_grid_frame = this->_transformation_service.transformToGridFrame(point);
...
geometry_msgs::Point OccupancyGridConversionService::convertToMapPoint(AStar::GridPoint grid_point) {
